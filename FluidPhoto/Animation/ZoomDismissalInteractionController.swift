...
class ZoomDismissalInteractionController: NSObject {
    ...
    func didPanWith(gestureRecognizer: UIPanGestureRecognizer) {
        ...
        guard let transitionContext = self.transitionContext,
            ...
            let fromReferenceImageView = animator.fromDelegate?.referenceView(for: animator),
            let toReferenceImageView = animator.toDelegate?.referenceView(for: animator),
...
extension ZoomDismissalInteractionController: UIViewControllerInteractiveTransitioning {
    func startInteractiveTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let animator = self.animator as? ZoomAnimator,
            let fromVC = transitionContext.viewController(forKey: .from),
            let toVC = transitionContext.viewController(forKey: .to),
            let fromReferenceImageViewFrame = animator.fromDelegate?.getFromCellFrame(for: animator),
            let toReferenceImageViewFrame = animator.toDelegate?.getFromCellFrame(for: animator)
            else {
                return
        }
        ...
        if animator.transitionImageView == nil {
			...
			transitionImageView.backgroundColor = .green
            transitionImageView.frame = fromReferenceImageViewFrame
